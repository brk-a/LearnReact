...
const TopChartCard = ({song, i, isPlaying, activeSong, handlePause, handlePlay}) => (
  <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
    ...
    <div className="flex-1 flex flex-row justify-between items center">
      <img src={song?.images?.coverart} alt={song?.title} className="w-20 h-2- rounded-lg"/>
    ...
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      ...
      <div className="w-full flex flex-col">
        ...
        <div className="mt-4 flex flex-col gap-1">
          {topPlays?.map((song, i) => (
            <TopChartCard key={song.key} song={song} i={i} isPlaying={isPlaying} activeSong={activeSong} handlePause={handlePause} handlePlay={() => handlePlay(song, i)} className=''/>
          ))}
        </div>
