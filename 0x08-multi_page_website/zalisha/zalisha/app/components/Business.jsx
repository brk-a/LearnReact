...
const FeatureCard = ({icon, title, content, index}) => {
  return(
    ...
    <div className={`w-[64px] h-[64-px] rounded-full ${styles.flexCenter} bg-dimBlue`}>
      <Image src={icon} alt={title} className='w-[50%] h-[50%] object-contain'/>
    </div>
    ...
    <section id="features" className={layout.section}>
      ...
      <div className={layout.sectionInfo}>
        <h2 className={styles.heading2}>
          You handle the business <br className="sm:block hidden"/> we'll handle the money.
        </h2>
        <p className={`${styles.paragraph} max-w-[470px] mt-5`}>
          With the right credit card, you can improve your
          financial life by building credit, earning rewards
          and saving money. However, with hundreds of credit
          cards in the market,
        </p>
        <Button styles='mt-10'/>
      </div>
